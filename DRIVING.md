...
Every endpoint, grouped by what it's for. All are HTTP **GET**; parameters are query strings. Every
response is JSON with an `ok` boolean.
...
| `/goto_wp`   | `name range`                        | Pathfind to a saved waypoint. |
| `/follow`    | `name range`                        | Continuously follow a player. |
| `/unfollow`  | —                                   | Stop following. |
| `/stop`      | —                                   | Cancel current path / clear movement. |
...
| `/craft`     | `item count`                        | Craft (auto-uses a nearby crafting table). |
| `/place`     | `name`                              | Place a block on an open spot beside you. |
| `/placeitem` | `name x y z`                        | Place a held item at an exact coordinate. |
| `/digat`     | `x y z`                             | Dig one specific block (no-op if air). |
| `/equip`     | `name dest`                         | Equip an item (dest defaults to `hand`). |
| `/lookat`    | `x y z`                             | Aim the head at a point. |
| `/chest`     | —                                   | Open + list the nearest chest (within 12). |
| `/withdraw`  | `name count`                        | Take items from the nearest chest. |
| `/smelt`     | `item fuel count`                   | Walk to nearest furnace, load input+fuel, wait, collect output. |
| `/chat`      | `msg`                               | Say something in chat (URL-encode it). |
| `/activate`  | `x y z`                             | Right-click a specific block (LOS-gated): beds (claim spawn / sleep at night), levers, buttons. Walks into reach. |
...
|-------------|-------------------|-------|
| `/state`    | —                 | Position, yaw/pitch, health, food, oxygen, held item, block looked at. |
| `/inventory`| —                 | Items (name/count/slot) + empty slot count. |
| `/scene`    | `verbose`         | One-line situational summary: facing, biome, phase, HP/food, ahead, entities, threats, resources. |
| `/map`      | `radius verbose`  | Top-down ASCII minimap (radius ≤ 40). |
| `/find`     | `name count radius xray` | Nearest matching blocks, coord + dist, nearest-first. **Only exposed + in-line-of-sight blocks** (no x-ray); `xray=1` bypasses for debug only — see §1. |
| `/entities` | `radius`          | Nearby entities (type/name/pos/dist), nearest-first. |
| `/gaze`     | `at\|dir dist`    | Narrated first-person look via real eye-rays (aims the head; feeds the SEEN memory + draws /map). |
| `/section`  | `dir len up down` | Vertical cross-section slice + narration: floor/ceiling profile, water, ore, sealed-vs-continues. |
| `/passages` | `radius`          | Exits from the current air pocket by direction (+up/down) **with a walk-to sample cell per exit** (probe them with /goto directly), or `closed pocket`. All `(sensed)`. |
| `/listen`   | `window`          | Aggregated recent sounds (ears). Danger tier (lava/fuse/explosion) self-announces via `[event] hear`. |
| `/boot`     | —                 | One-call session opener: body + tools (durability) + sky + jobs + chat cursor. |
| `/gesture`  | `name at`         | nod / shake / wave / point?at=x,y,z — nonverbal co-op presence. |
| `/snapshot` | `w h`             | Best-effort first-person PNG (often unavailable — fails cleanly). `/gaze` is the primary look. |

### 🧱 Building — structures
| Endpoint   | Params (async as noted)                          | Builds |
|------------|--------------------------------------------------|--------|
| `/build`   | `template=cabin x z w d h base name base_name` — **async job** | Full cabin: foundation → walls → doorway → roof. |
| `/outline` | `x z w d name y`                                 | Rectangular foundation perimeter on the ground. |
| `/walls`   | `x z w d h base name base_name door`             | Perimeter walls to height `h` with a doorway. |
| `/roof`    | `x z w d y name`                                 | Solid slab at height `y`, outer-ring-first. |

### 🧠 Memory — persist across sessions
| Endpoint     | Params        | Remembers |
|--------------|---------------|-----------|
| `/waypoint`  | `name x y z`  | Store a named coord (or stamp current spot; or read one back). |
| `/waypoints` | `verbose`     | List all saved waypoints. |
| `/goto_wp`   | `name range`  | Navigate to a saved waypoint (also listed under Doing). |
| `/journal`   | `n`           | Last `n` lines of the on-disk journal. |

### 📡 Awareness — chat & events (the heartbeat)
| Endpoint    | Params  | Streams |
|-------------|---------|---------|
| `/chatlog`  | `since` | New chat/whisper messages since a cursor (returns new `cursor`). |
| `/events`   | `since` | New interrupt events since a cursor (damage, hostile, nightfall, low_food, job progress…). |
| `/jobs`     | —       | All background jobs (id/name/status/note). |
| `/job`      | `id verbose` | One job's status + latest progress (+ result when done). |
